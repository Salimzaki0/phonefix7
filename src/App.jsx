...
function estimatePrice(brandId, issueId) {
  const issue = ISSUES.find((i) => i.id === issueId);
  const mult = BRAND_MULT[brandId] || 1;
  const low = Math.round(issue.base * mult);
  const high = Math.round(issue.base * mult * 1.6);
  return { low, high };
}

function buildWhatsAppMessage({ ticketNo, brandLabel, model, issueLabel, price, name, phone, city }) {
  const lines = [
...
function TicketLine({ label, value, filled }) {
  return (
    <div className="flex items-baseline justify-between border-b border-dashed border-[#3a3f38] py-2">
      <span className="text-[11px] tracking-widest uppercase text-[#8a8f86]" style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
        {label}
      </span>
      <span
        className={`text-sm ${filled ? "text-[#EDEAE3]" : "text-[#4a4f47]"}`}
...
export default function RepairShop() {
  ...
  const price = useMemo(() => (brand && issue ? estimatePrice(brand, issue) : null), [brand, issue]);
  ...
  // total visits: count this visit once per mount
  useEffect(() => {
    (async () => {
      try {
        let count = 1;
        try {
          const res = await window.storage.get(TOTAL_VISITS_KEY, true);
          count = (res ? parseInt(res.value, 10) || 0 : 0) + 1;
        } catch (e) {
          count = 1;
        }
        await window.storage.set(TOTAL_VISITS_KEY, String(count), true);
  ...
  // live viewers: heartbeat presence in a shared session map
  useEffect(() => {
    let cancelled = false;

    async function beat() {
      try {
        const res = await window.storage.get(ACTIVE_SESSIONS_KEY, true);
        const map = res ? JSON.parse(res.value) : {};
        const now = Date.now();
        map[sessionId] = now;
        for (const key of Object.keys(map)) {
          if (now - map[key] > ACTIVE_WINDOW_MS) delete map[key];
        }
        await window.storage.set(ACTIVE_SESSIONS_KEY, JSON.stringify(map), true);
  ...
  async function submit() {
    ...
    const msg = buildWhatsAppMessage({ ticketNo, brandLabel, model, issueLabel, price, name, phone, city });
        ...
        <div className="w-full max-w-4xl grid md:grid-cols-[1.15fr_0.85fr] gap-6">
          <div className="rounded-2xl p-6 md:p-9" style={{ background: "#1E2226", border: "1px solid #2c3136" }}>
            <h1 className="text-2xl md:text-[28px] font-bold mb-6" style={{ color: "#EDEAE3" }}>
              کێماسیا مووبایلا خۆ دیاگنۆز بکە
            </h1>
            ...
            {!submitted ? (
              <>
                ...
                {step === 0 && (
                  <div>
                    <p className="text-sm mb-4" style={{ color: "#a7ada3" }}>مارکا مووبایلا تە کیجا یە؟</p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {BRANDS.map((b) => (
                        <button
                          ...
                          className="rounded-xl px-4 py-3 text-sm text-right transition-all"
                          style={{
                            background: brand === b.id ? "#E8A33D14" : "#181c1f",
                            border: brand === b.id ? "1px solid #E8A33D" : "1px solid #2c3136",
                            color: brand === b.id ? "#E8A33D" : "#c9cdc6",
                          }}
                        >
                          {b.label}
                        </button>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-72 overflow-y-auto pr-1">
                        {IPHONE_MODELS.map((m) => (
                          <button
                            key={m}
                            onClick={() => setModel(m)}
                            className="rounded-lg px-3 py-2.5 text-xs text-right transition-all"
                            style={{
                              background: model === m ? "#E8A33D14" : "#181c1f",
                              border: model === m ? "1px solid #E8A33D" : "1px solid #2c3136",
                              color: model === m ? "#E8A33D" : "#c9cdc6",
                      ...
                      {ISSUES.map((it) => {
                        ...
                        return (
                          <button
                            ...
                            className="rounded-xl px-4 py-4 text-sm flex flex-col items-start gap-3 transition-all"
                            style={{
                              background: active ? "#E8A33D14" : "#181c1f",
                              border: active ? "1px solid #E8A33D" : "1px solid #2c3136",
                ...
                {step === 3 && (
                  <div>
                    <p className="text-sm mb-4" style={{ color: "#a7ada3" }}>
                      زانیاریێن پەیوەندیێ دەرباز کە، ئەم دگەهنە دەڤ تە
                    </p>
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        placeholder="شار / ناڤچە (ئارەزوومەندانە)"
                        className="w-full rounded-lg px-4 py-3 text-sm outline-none"
                        style={{ background: "#181c1f", border: "1px solid #2c3136", color: "#EDEAE3" }}
                      />
                    </div>
